...
{                              //先来先服务调度算法
    ...
    while (1)
    { //如果当前没有正在运行进程，并且输入队列和就绪队列都为空，则所有进程完成
        if (!running_process && input_queue.empty() && ready_queue.empty())
            break;

        while (!input_queue.empty())
        { //将到达时间小于等于当前时间片的进程从输入队列移到就绪队列中
...
{ //时间片轮转法
    ...
    while (1)
    { //如当前无运行进程，同时输入队列和就绪队列都为空，则所有进程完成
        if (!running_process && input_queue.empty() && ready_queue.empty())
        {
            break;
        }
...
void dynamic_prio()
{
    ...
    { //如果当前没有正在运行进程，并且输入队列和就绪队列都为空，则所有进程完成
        ...
        { //当前运行任务完成一个时间片，判断任务是否完成，更改其信息
            ...
            { //任务未完成，如果就绪队列仍有任务，且优先级大于本任务优先级，则轮转调度，否则不调度
